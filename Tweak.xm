    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:[(NSString *)fp0Ref autorelease]]) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            @autoreleasepool {
                ...
                // Get the MD5 of the dylib locally, and remote MD5s
                // Different versions will have different MD5s. 
                // Hence, we need to iterate through all of them.
                ...
                if (allTheShitRef) {
                    NSLog(@"QS: Array exists, iterating");
                    for (NSString *realStuffRef in (NSArray *)allTheShitRef) {
                        if ((CFStringCompare(theStuffRef, (CFStringRef)realStuffRef, 0) == kCFCompareEqualTo)) {
                            NSLog(@"QS: The stuff checked out: %@, %@", (NSString *)theStuffRef, (NSString *)realStuffRef);
                            done = 23 << 2;
                            break;
                        }
                        else {
